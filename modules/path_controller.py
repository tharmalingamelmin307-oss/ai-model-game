...
class CarAvoidancePdController:
    """车辆避障专用 PD 控制器."""
    ...
    def __init__(self):
        ...
        self.last_pd_pwm = 0.0
        self.last_sample_y = None
        self.pd_error = None
        self.last_blocked_y_range = None
    ...
    def _base_debug(self, miss_frames=0):
        return {
            ...
            "pass_latched": bool(self.pass_latched),
            "active": False,
        }

    def idle_debug(self, miss_frames=0):
        return self._base_debug(miss_frames=miss_frames)

    def begin_clearing(self, passed=False):
    ...
    def _clearing_progress(self):
        decay_frames = max(1, int(getattr(config, "CAR_AVOIDANCE_CLEARING_DECAY_FRAMES", 5)))
        if decay_frames <= 1:
            return 1.0
        progress_frames = max(0.0, float(self.clear_frames) - 1.0)
        return float(np.clip(progress_frames / float(decay_frames - 1), 0.0, 1.0))

    def _clearing_done(self, progress):
        done_frames = max(1, int(getattr(config, "CAR_AVOIDANCE_CLEARING_DECAY_FRAMES", 5)))
        max_clear_frames = max(
            done_frames,
            int(getattr(config, "CAR_AVOIDANCE_CLEARING_MAX_FRAMES", done_frames)),
        )
        return (self.clear_frames >= done_frames and float(progress) >= 1.0) or self.clear_frames >= max_clear_frames

    def compute_pd_pwm(self, left_x, w_seg, inset_x):
        if left_x is None:
            return None, None, None, 0.0, 0.0

        target_x = float(w_seg) * float(getattr(config, "CAR_AVOIDANCE_LEFT_BOUNDARY_TARGET_RATIO", 0.50))
        control_x = float(left_x) + float(inset_x)
        deadband = max(0.0, float(getattr(config, "CAR_AVOIDANCE_LEFT_BOUNDARY_PD_DEADBAND", 4.0)))
        raw_error = float(control_x) - target_x
        ...
        pd_pwm = p_term + d_term

        self.pd_error = float(error)
        return float(pd_pwm), raw_error, float(left_x), float(p_term), float(d_term)

    def update_missing(self, left_x, half_width_x, w_seg, miss_frames=0):
        if self.state == "AVOIDING":
            self.begin_clearing(passed=False)
        elif self.state == "PASSED_WAIT":
            self.reset()

        debug = self._base_debug(miss_frames=miss_frames)
        if self.state != "CLEARING":
            return debug

        self.clear_frames += 1
        progress = self._clearing_progress()

        near_inset = max(0.0, float(getattr(config, "CAR_AVOIDANCE_PD_NEAR_INSET_X", 10.0)))
        half_width = max(near_inset, float(half_width_x)) if half_width_x is not None else near_inset
        inset_x = near_inset + (half_width - near_inset) * progress
        pd_pwm = float(self.last_pd_pwm)
        left_p_pwm = 0.0
        left_d_pwm = 0.0
        left_boundary_error = None
        left_boundary_x = None
        sample_pd_pwm, left_boundary_error, left_boundary_x, left_p_pwm, left_d_pwm = self.compute_pd_pwm(
            left_x,
            w_seg,
            inset_x,
        )
        if sample_pd_pwm is not None:
            pd_pwm = float(sample_pd_pwm)

        self.last_pd_pwm = float(pd_pwm)
        debug.update({
            "state": self.state,
            "clear_frames": int(self.clear_frames),
            "boundary_inset_x": float(inset_x),
            "boundary_strength_x": float(inset_x),
            "left_boundary_error": None if left_boundary_error is None else float(left_boundary_error),
            "left_boundary_x": None if left_boundary_x is None else float(left_boundary_x),
            "left_boundary_p_pwm": float(left_p_pwm),
            "left_boundary_d_pwm": float(left_d_pwm),
            "pd_pwm": float(pd_pwm),
            "active": True,
        })
        if self._clearing_done(progress):
            if self.pass_latched:
                self.state = "PASSED_WAIT"
                self.clear_frames = 0
                self.last_pd_pwm = 0.0
                self.pd_error = None
            else:
    ...
    def update_locked(self, *, rows_to_car, boundary_ready, left_x, half_width_x, w_seg, sample_y, miss_frames=0, blocked_y_range=None):
        debug = self._base_debug(miss_frames=miss_frames)
        debug["rows_to_bottom"] = float(rows_to_car)

        if self.state == "CLEARING":
            return self.update_missing(left_x, half_width_x, w_seg, miss_frames=miss_frames)
        if self.state == "PASSED_WAIT":
            return debug
        ...
        if not boundary_ready:
            ...
            debug["clear_frames"] = int(self.clear_frames)
            return debug

        near_rows = max(0.0, float(getattr(config, "CAR_AVOIDANCE_NEAR_BOUNDARY_ROWS", 60.0)))
        far_inset = max(0.0, float(getattr(config, "CAR_AVOIDANCE_PD_FAR_INSET_X", 25.0)))
        near_inset = max(0.0, float(getattr(config, "CAR_AVOIDANCE_PD_NEAR_INSET_X", 10.0)))
        inset_x = near_inset if float(rows_to_car) <= near_rows else far_inset
        if half_width_x is not None:
            inset_x = min(float(inset_x), max(0.0, float(half_width_x)))
        ...
        pd_pwm = float(self.last_pd_pwm) if sample_pd_pwm is None else float(sample_pd_pwm)
        self.state = "AVOIDING"
        self.clear_frames = 0
        self.last_sample_y = float(sample_y)
        self.last_pd_pwm = float(pd_pwm)
        self.last_blocked_y_range = blocked_y_range
