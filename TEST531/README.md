...
1. 对 `SEG_SIZE` 小图执行分割
2. 从分割输出中得到二值赛道 `mask`
3. 自底向上逐层扫描，寻找每一层的白色连通段中心
4. 把上下层能接起来的中心点连成多条候选路径
5. 对候选路径按长度、平滑度、局部中心偏离做打分
6. 如果检测到 `stone`
   优先绕开石头更接近的那一支
7. 如果没有明确石头干预
   默认偏向左支
8. 如果有 `turn_intent`
   再用 `LEFT / RIGHT` 覆盖默认偏向
9. 对最终路径做多项式拟合
10. 对拟合系数做 EMA 平滑
11. 将拟合路径转换成单一 `steer_signal`，并渲染调试图
...
- 当前代码里虽然保留了 `stone` 投影和左右分支判断的辅助函数，但石头绕行还主要停留在调试/预留阶段
- 真正直接参与主分支选择的，当前仍然是“候选路径打分 + `turn_intent` 左右偏向覆盖”
...
这几个参数当前保留着，但主流程没有直接使用，更多是给后续实验路径预留。
...
4. 实际写入的限速是 `识别数字 - 1`。
5. `MOTOR_MAX_SPEED` 当前更多是配置保留位，串口线程实际使用的是内部 10~30 档速度逻辑。
6. `OCR_CROP_EXPAND_RATIO / OCR_MIN_CROP_HEIGHT / ROI_TOP_CUT_RATIO` 当前仍是预留参数，不直接参与主流程。
