...
class YOLODetector:
    def __init__(self, core_id):
        ...
        self.expect_raw_single_output = bool(config.YOLO_EXPECT_RAW_SINGLE_OUTPUT)
    ...
    def _preprocess(self, frame):
        ...
        mean = np.array(config.YOLO_PREPROCESS_MEAN, dtype=np.float32)
        std = np.array(config.YOLO_PREPROCESS_STD, dtype=np.float32)
        img = (img - mean) / std
    ...
    def _decode_single_tensor_outputs(self, outputs, output_w, output_h, input_w, input_h):
        ...
        is_cxcywh = bool(config.YOLO_SINGLE_TENSOR_IS_CXCYWH)
        if is_cxcywh:
