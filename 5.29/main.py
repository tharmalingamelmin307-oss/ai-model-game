...
def throttled_log(key, message, state=None, min_interval=1.5):
...
def extract_scene_control_signals(boxes):
    ...
    zebra_cls_id = config.CLASS_NAMES.index("zebra_crossing") if "zebra_crossing" in config.CLASS_NAMES else 5
    red_cls_id = config.CLASS_NAMES.index("traffic_light_red") if "traffic_light_red" in config.CLASS_NAMES else 6
    green_cls_id = config.CLASS_NAMES.index("traffic_light_green") if "traffic_light_green" in config.CLASS_NAMES else 7
    yellow_cls_id = config.CLASS_NAMES.index("traffic_light_yellow") if "traffic_light_yellow" in config.CLASS_NAMES else 8
...
def draw_zebra_stopline(image, zebra_stopline):
    ...
    extend_ratio = float(getattr(config, "ZEBRA_STOPLINE_EXTEND_RATIO", 0.35))
    extend_w = w * extend_ratio
    ...
    cv2.line(image, (x1, y), (x2, y), (0, 255, 255), 3, cv2.LINE_AA)
    return image
    ...
    while True:
        try:
            ...
            for idx, obj in enumerate(objs):
                ...
                if cls_id not in (
                    getattr(config, "SIGN_CLASS_ID", 9),
                    getattr(config, "LIMIT_SIGN_CLASS_ID", 10),
                ):
                    continue
                rect = obj.get("rect", [0, 0, 0, 0])
    ...
    while True:
        try:
            ...
            with data_lock:
                ...
                sign_class_id = getattr(config, "SIGN_CLASS_ID", 9)
                limit_sign_class_id = getattr(config, "LIMIT_SIGN_CLASS_ID", 10)
        ...
        if rendered_img is not None:
            ...
            if traffic_light_state:
                cv2.putText(
                    ...
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.42,
                    (0, 255, 255) if traffic_light_state == "yellow" else ((0, 255, 0) if traffic_light_state == "green" else (0, 0, 255)),
                    1,
                    cv2.LINE_AA
                )
...
def serial_control_thread():
    ...
    while True:
        ...
        try:
            ...
            stop_trigger_dist = int(getattr(config, "ZEBRA_STOPLINE_TRIGGER_DIST", 110))
            stop_ready = (
                zebra_stopline_y is not None and
                (config.TARGET_RES[1] - int(zebra_stopline_y)) <= stop_trigger_dist
            )
        ...
        if control_debug != last_control_debug:
            ...
            if traffic_light_state in ("red", "yellow", "green"):
                zebra_dist_text = "无" if zebra_stopline_y is None else str(config.TARGET_RES[1] - int(zebra_stopline_y))
                throttled_log(
                    "traffic_stop_detail",
                    "红绿灯停车判定: "
